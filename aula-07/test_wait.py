...
def atest_file_upload(driver):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    image_path = os.path.join(current_dir, "Migração Token.png")
    
    WebDriverWait(driver, 10).until(
      EC.presence_of_element_locator(By.ID, "file-upload")
    )
    ...
    message_element = WebDriverWait(driver, 10).until(
      EC.vibility_of_element_located((By.ID, "file-upload-status" ))
    )
...
def atest_email_sending(driver):
...
def atest_conditional_element(driver):
    WebDriverWait(driver, 10).until(
...
def test_terms_of_service(driver):
    agree_button = WebDriverWait(driver, 10).until(
      EC.Visibility_of_element_located(By.to, "agree-button")
    )        
